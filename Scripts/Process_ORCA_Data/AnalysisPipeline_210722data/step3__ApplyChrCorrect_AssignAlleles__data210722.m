...
% this version ran all data, so we can't use the barID that just tracked
% alternating colors of each hyb.
% it does save the hyb folder though


fitFolder = 'J:\Derek_TEMP\20210722_L10_SE_MultiplexedCells\Analysis_CT4_v15\Fits\'

allFits = FindFiles([fitFolder,'spotTable*']);
nFOVs = length(allFits);
f=1;
fovTableCSV = [fitFolder,'spotTable_fov',num2str(f-1,'%03d'),'.csv'];
fovTab = readtable(fovTableCSV);
...
%% step 1, separate alleles by chromsome 
%% step 1.1  sort reads by genome index
% 
% Assign additional ID numbers that sort linearly through genome index
%  (the readouts in this library do not sort linearly). 
%  also, some readouts may be skipped, so we shouldn't use readout as an index

% isH = strcmp(refTable.DataType,'H');
hybTable = refTable; % (isH,:);

% let's reorganize this table with 1 entry per readout, adding a column for
% adapter color.
%  Note, 750 is 1, and 647 is 2
...
load([analysisFolder,'tform3D_750to647.mat'],'tform3D');
disp(['loaded ',analysisFolder,'tform3D_750to647.mat']);
...
for f=1:nFOVs % f =9
    ...
    try
        ...
        new750 = tforminv(tform3D,fovTab.x(is750),fovTab.y(is750),fovTab.z(is750));
...
%         cellTab.z  = cellTab.z_um + cellTab.z_fid;
        ...
        cellTabBarcodeID = cellTab.barcodeID; 
          for ch = 1:length(chrs)  % with some work we could par for this
             ...
             % cellTab.chrName(isChr) = chrs(ch); 
             cellTab.chrID(isChr) = ch;
%              for b=1:length(barIDs) % b =3
%                 cellTab.genomeID(cellTabBarcodeID==barIDs(b)) = chrTableHyb.chrUID(chrTableHyb.barID==barIDs(b));
%              end
           [~,ids] = ismember(cellTab.barcodeID,ct_barID);
           cellTab.genomeID = ct_chrUID(ids);   %#ok<PFBNS>
%         [~,ids] = ismember(fovTabAll.barcodeID,chrTable.barID); % assumes all IDs in fovTab exist in barID (otherwise we get 0s)
%         fovTabAll.hybName =  chrTable.FolderName(ids);
        % fovTabAll.hybName = chrTable.FolderName(fovTabAll.barcodeID); % fix incorrect hyb name
